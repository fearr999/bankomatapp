...
## Платформа нового поколения (в разработке)

В директории `platform/` строится полноценная FSM-платформа (веб-панель для
диспетчера/руководителя + новый backend на Postgres) — см.
[`platform/README.md`](platform/README.md) для стека, что уже готово и что
дальше по дорожной карте. Текущее Flutter-приложение и Apps Script выше
продолжают работать без изменений, пока платформа разрабатывается отдельно.
